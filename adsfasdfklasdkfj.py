...
def game_start():
    ...
    def opponent_character_selection():  # Used to generate a pokemon to the opponent
        opponent_selection = rm.choice(list(
            pokemon_dictionary.keys()))  # Edit and put the letter of your choice pokemon if you want it to be set manually rather than randomly (original = rm.choice(list(pokemon_dictionary.keys())) )
    ...
    def power_computation(player_pokemon, opponent_pokemon, player_power_base,
                          opponent_power_base):  # This is used to compute for a new power base if either the opponent or the player has been lost. If the power is 0, they will be having the base power of their pokemon, else they will have the current power of their pokemon.
    ...
    def element_interaction_buffsandnerfs(player_pokemon, opponent_pokemon, player_power_base,
                                          opponent_power_base):  # Used for computing the random variation to the power of the pokemon
        ...
        if (opponent_pokemon[2] in type_interactions[player_pokemon[2]][0]) or (
                opponent_pokemon[2] in type_interactions[player_pokemon[2]][1]):
    ...
    def battle(player_pokemon, opponent_pokemon, final_player_power, final_opponent_power, voicelines=voicelines,
               ending_voicelines=ending_voicelines):  # Used for simulating the battle, showing voicelines and the conclusion of the battle
        ...
        if final_player_power > final_opponent_power:
            ...
            typing_effect("As a result, your pokemon will now absorb the power base of the opponent's pokemon",
                          end=True)
            for i in range(3):
                t.sleep(0.8)
                print(".", end="", flush=True)
            ...
            t.sleep(0.8)
            typing_effect(
                f"Opponent's Pokemon ({opponent_pokemon[0]}) has a total of {post_opponent_power_base} base power!")
            battle_outcome = "lost"
    ...
    def play(player_pokemon, opponent_pokemon, player_power_base, opponent_power_base, match_id):
        ...
        player_power_base, opponent_power_base = power_computation(player_pokemon=player_pokemon,
                                                                   opponent_pokemon=opponent_pokemon,
                                                                   player_power_base=player_power_base,
                                                                   opponent_power_base=opponent_power_base)
        final_player_power, final_opponent_power, player_buffornerf, opponent_buffornerf = element_interaction_buffsandnerfs(
            player_pokemon=player_pokemon, opponent_pokemon=opponent_pokemon, player_power_base=player_power_base,
            opponent_power_base=opponent_power_base)
        battle_outcome, post_player_power_base, post_opponent_power_base = battle(player_pokemon=player_pokemon,
                                                                                  opponent_pokemon=opponent_pokemon,
                                                                                  final_player_power=final_player_power,
                                                                                  final_opponent_power=final_opponent_power)
        battle_history.append([match_id, player_pokemon[0], player_power_base,
                               f"+{player_buffornerf}" if player_buffornerf > 0 else player_buffornerf,
                               final_player_power, opponent_pokemon[0], opponent_power_base,
                               f"+{opponent_buffornerf}" if opponent_buffornerf > 0 else opponent_buffornerf,
                               final_opponent_power, battle_outcome, post_player_power_base,
    ...
    def main_menu():
        ...
        while True:
            ...
            if user_input.lower() == "a":
                ...
                while True:
                    player_pokemon, opponent_pokemon, player_power_base, opponent_power_base, battle_outcome, match_id = play(
                        player_pokemon=player_pokemon, opponent_pokemon=opponent_pokemon,
                        player_power_base=player_power_base, opponent_power_base=opponent_power_base, match_id=match_id)
                    while True:
                        ...
                        if user_choice.lower() == "m":
                            print(tabulate(battle_history, headers=table_head,
                                           tablefmt="pretty"))  # Process of tabulating the data we currently have
                            ...
                            while True:
                                typing_effect("Are you sure?")
                                typing_effect(
                                    "WARNING: Restarting will reset both the pokemon and power bases of both the player and opponent")
                                print("Press the key of your preferred selection:\n'b' : Go back\n'p' : Proceed")
                                change_input = input("User Input: ")
                                if change_input.lower() == "b":
                                    break
                            ...
                            while True:
                                typing_effect("Are you sure?")
                                typing_effect(
                                    "WARNING: Going back to the main menu will reset the player and the opponent's pokemon and power bases")
                                print("Press the key of your preferred selection:\n'b' : Go back\n'p' : Proceed")
                                menu_choice = input()
